...
@csrf
        ...
        <div>
            ...
            <select id="type" name="type"
                ...
                @foreach (App\Enums\TalkType::cases() as $type)
                    <option value="{{ $type->value }}" 	@selected($type->value === old('type', $talk->type)) >{{ ucwords($type->value) }}</option>
                @endforeach
            </select>
    ...
    <div>
        ...
        <textarea id="abstract" name="abstract" rows="4" value="{{ old('abstract', $talk->abstract) }}"
    ...
    <div>
        ...
        <textarea id="organizer_notes" name="organizer_notes" rows="4" value="{{ old('organizer_notes', $talk->organizer_notes) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"></textarea>
